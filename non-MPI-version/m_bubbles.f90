...
MODULE m_bubbles
  ...
  FUNCTION f_vflux( R,V,mass_v )
    ...
    IF ( vapor=='y' ) THEN
       ...
       IF ( thermal=='transfer' ) THEN
          ...
          f_vflux = rho_mw*grad_chi/Pe_c/( 1.D0-chi_vw )/R
       ELSE
  ...
  FUNCTION f_bpres_dot( vflux,R,V,pb,mass_v )
    ...
    IF ( thermal=='transfer'.AND.model=='Preston' ) THEN
       ...
       f_bpres_dot = 3.D0*gamma_m*( -V*pb+vflux*R_v*Tw &
                   + pb0(iR0)*k_mw*grad_T/Pe_T(iR0)/R )/R
    ELSE IF ( thermal=='transfer'.AND.model=='Sugiyama' ) THEN
       ! only for gas bubble
       T_bar = Tw*( pb/pb0(iR0) )*( R/R0(iR0) )**3
       tmp1 = -V*pb - pb0(iR0)*k_mw/Pe_T(iR0)/R &
    ...
    ELSE
       f_bpres_dot = -3.D0*gamma_m*V/R*( pb-pv )
    END IF
  ...
  FUNCTION f_bwpres_1( R,V,pb, ir )

    REAL(KIND(0.D0)), INTENT(IN) :: R
    REAL(KIND(0.D0)), INTENT(IN) :: V
    REAL(KIND(0.D0)), INTENT(IN) :: pb
    INTEGER, INTENT(IN) :: ir !SHB: fix memory leak 
    REAL(KIND(0.D0)) :: tmp
    REAL(KIND(0.D0)) :: f_bwpres_1

    IF ( polytropic=='y' ) THEN
       tmp = ( R0(iR)/R )**( 3.D0*gamma_m )
       tmp = ( Ca+2.D0/We/R0(iR) )*tmp - Ca + 1.D0
  ...
  FUNCTION f_enthal( Cpinf,Cpbw )
    ...
    !print*, 'H = ', ( tmp2-tmp3 )*n_tait*( 1.D0+B_tait )/( n_tait-1.D0 )
  ...
  FUNCTION f_enthal_dot( R,V,Cpbw,Cpinf,pb_dot,Cpinf_dot )
    ...
    IF ( polytropic=='y' ) THEN
       tmp1 = ( R0(iR0)/R )**( 3.D0*gamma_m )
       tmp1 = -3.D0*gamma_m*( Ca+2.D0/We/R0(iR0) )*tmp1*V/R
    ELSE IF ( polytropic=='n' ) THEN
       tmp1 = pb_dot
    END IF
  ...
  FUNCTION f_sound_SEOS( Cpinf )

    REAL(KIND(0.D0)), INTENT(IN) :: Cpinf
    REAL(KIND(0.D0)) :: f_sound_SEOS

    f_sound_SEOS = Sqrt( n_tait*(Cpinf+B_tait)/rhol0 )

  END FUNCTION f_sound_SEOS

  !========================================================================

  !returns \ddot{R} for the Gilmore equation using Tait EOS
  FUNCTION f_gilmore( R,V,pb,pb_dot,pinf,pinf_dot )
    !if polytropic, pb and pb_dot = 0, pinf is local pressure and pinf_dot is pldot
    REAL(KIND(0.D0)), INTENT(IN) :: R
    REAL(KIND(0.D0)), INTENT(IN) :: V
    REAL(KIND(0.D0)), INTENT(IN) :: pb
    REAL(KIND(0.D0)), INTENT(IN) :: pb_dot
    ...
    tmp3 = 1.5D0*V**2*( tmp1/3.D0-1.D0 ) + H*( 1.D0+tmp1 ) &
     ...
     !print*, 'h, hdot', h, h_dot
     !print*, 'rddot', tmp3/( R*(1.D0-tmp1)*tmp2 )


    f_gilmore = tmp3/( R*(1.D0-tmp1)*tmp2 )
    !f_gilmore = (H - 1.5*V*V)/R

  END FUNCTION f_gilmore


  !========================================================================
  !returns \ddot{R} for the Keller-Miksis equation using Stiffened gas EOS
  FUNCTION f_KM( R,V,pb,pb_dot,pinf,pinf_dot )
    ...
    t3 = (1. - V/A)
    t4 = (pb - pinf)/rhol0
    t5 = (R/(rhol0*A))*(pb_dot - pinf_dot)

    f_KM = 1./(t3*R)*( t2*t4 + t5 - t1 )
    !tmp3/( R*(1.D0-tmp1)*tmp2 )
