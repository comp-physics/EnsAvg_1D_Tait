...
MODULE m_globalvar
  ...
  ! WENO order
  INTEGER, PARAMETER :: wenoord = 5
  INTEGER, PARAMETER :: wenonum = ( wenoord-1 )/2
  ...
  CHARACTER(LEN=1) :: timesplit
  CHARACTER(LEN=1) :: mpweno
  CHARACTER(LEN=1) :: artcomp
  ...
  TYPE(coordinate), DIMENSION(:), ALLOCATABLE :: dqbub

  ! time increment used for adaptive RK
  TYPE(coordinate), DIMENSION(:), ALLOCATABLE :: dttry
  ...
  ! for WENO3 (used for nonreflecting BCs)
  ...
  !SHB Stiffened gas stuff
  REAL(KIND(0.D0)) :: rhol0 !let's just make this global
  ...
  ! for polytropic gas, gamma_m represents polytropic index
  REAL(KIND(0.D0)) :: gamma_m
  ! molecular weight
  REAL(KIND(0.D0)) :: M_n
  REAL(KIND(0.D0)) :: M_v
