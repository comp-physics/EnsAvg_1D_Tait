...
MODULE m_rhsvar
  ...
  ! WENO3 used for non-reflecting BCs
  ...
  ! numerical flux at celledges
  TYPE(coordinate), DIMENSION(:), ALLOCATABLE :: nfx
  ! numerical source flux in momentum equation at celledges
  REAL(KIND(0.D0)), DIMENSION(:), ALLOCATABLE :: nsfx
  ! cell-edge velocity
  REAL(KIND(0.D0)), DIMENSION(:), ALLOCATABLE :: velhf
  ! flux used in subroutines for non-reflecting BCs
  ...
  TYPE(coordinate), DIMENSION(:), ALLOCATABLE :: lac
  TYPE(coordinate), DIMENSION(:), ALLOCATABLE :: rac

  ! for characteristic decomposition
  TYPE(coordinate), DIMENSION(:), ALLOCATABLE :: v_out
  ! left & right eigenvectors at celledges
  ...
  SUBROUTINE s_start_rhsvar
    ...
    ! artificial compression
    IF ( artcomp=='y' ) THEN
       ALLOCATE( lac(Nart:Nv) )
       ALLOCATE( rac(Nart:Nv) )
       DO iv = Nart,Nv ! for linearly degenerate fields
          ALLOCATE( lac(iv)%f(1:N1) )
          ALLOCATE( rac(iv)%f(1:N1) )
       END DO
    END IF
  ...
  SUBROUTINE s_compvar
    ...
    ! characteristic speeds (eigenvalues) used for non-reflecting BCs
  ...
  SUBROUTINE s_multiply_ll( v_in,st,lr )
    ...
    !input v_in are conserved variables
    !output v_out are characteristic variables
    DO i = 1,N1
        !p = L.q
       ...
       v_out(Nveul)%f(i) = ll(4)%f(ip(i,lr))*v_in(1)%f(ip(i,st)) &
                         - v_in(Nveul)%f(ip(i,st))
       tmp = ll(5)%f(ip(i,lr))
       DO ib = 1,Nb     !go through number of bubble vars. Nb=2 if no heat/mass transfer
          DO ir = 1,NR0 !go through number of bubble radii
                v_out(ibub(ib,ir))%f(i) = &
  ...
  SUBROUTINE s_celledgevalue
    ...
    DO i = Nbeg,N1
       ...
       lflux(1)%f(i) = lqval(dir1)%f(i)  !\rho u
       lflux(dir1)%f(i) = lqval(dir1)%f(i)*lvel(i) + lpres(i) !\rho u + p
       DO iv = Nveul,Nv
          lflux(iv)%f(i) = lqval(iv)%f(i)*lvel(i) !\var * u
       END DO
    END DO
    DO i = 1,Nend
  ...
  SUBROUTINE s_hllwavespeed2

    INTEGER :: i

    ! redefine wavespeeds (see Johnsen 2007)
    DO i = Nbeg,N1
       sl(i) = MIN( 0.D0,sl(i) )
       sr(i) = MAX( 0.D0,sr(i) )
    END DO

  END SUBROUTINE s_hllwavespeed2
